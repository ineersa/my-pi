...
export function renderSubagentResult(
	...
	const stepsToShow = d.results.length;
	...
	for (let i = 0; i < stepsToShow; i++) {
		...
		const rProg: AgentProgress | undefined = r.progress || progressFromArray;
		const rSummary = r.progressSummary;
		...
		const stats = (rProg || rSummary) ? ` | ${(rProg || rSummary)!.toolCount} tools, ${formatDuration((rProg || rSummary)!.durationMs)}` : "";
