...
function isAssistantMessageEntry(entry: SessionLikeEntry): entry is SessionLikeMessageEntry {
  return entry.type === "message" && entry.message != null && entry.message.role === "assistant";
}
...
export default function rewindExtension(pi: ExtensionAPI) {
  ...
  async function reconstructState(ctx: ExtensionContext) {
    ...
    for (const entry of ctx.sessionManager.getBranch() as SessionLikeEntry[]) {
      const entryId = "id" in entry ? (entry as { id?: string }).id : undefined;
      const boundCommitSha = entryId ? entryToCommit.get(entryId) : undefined;
  ...
  async function discoverSessionFiles(scanMode: "ancestor-only" | "repo-sessions"): Promise<string[]> {
    ...
    if (scanMode === "repo-sessions") {
      ...
      while (stack.length > 0) {
        ...
        try {
          dirents = await readdir(dir, { withFileTypes: true }) as import("fs").Dirent[];
        } catch {
        ...
        for (const dirent of dirents) {
          const name = typeof dirent.name === "string" ? dirent.name : String(dirent.name);
          const fullPath = join(dir!, name);
          if (dirent.isDirectory()) {
            stack.push(fullPath);
            continue;
          }
          if (dirent.isFile() && name.endsWith(".jsonl")) {
  ...
  async function runRetentionSweep(ctx: ExtensionContext, reason: "startup" | "new-snapshots" | "shutdown") {
    ...
    for (const sessionFile of sessionFiles) {
      ...
      if (!ledger.cwd || !isInsidePath(ledger.cwd, repoRoot!)) continue;
      ledgers.push(ledger);
    }
