...
export function parsePolicyPatch(input: unknown): ParseResult<CompactionPolicyPatch> {
	...
	for (const [sectionKey, sectionValue] of Object.entries(input)) {
		...
		for (const [field, value] of Object.entries(sectionValue)) {
			if (sectionKey === "trigger" && field === "autoMaxTokens") {
				const parsed = parseAutoMaxTokens(value);
				if (!parsed.ok) {
					return { ok: false, error: `Invalid trigger.autoMaxTokens: ${parsed.error}` };
				}
				patch.trigger = { ...patch.trigger, autoMaxTokens: parsed.value };
				continue;
			}
...
function parseAutoMaxTokens(value: unknown): ParseResult<AutoMaxTokensConfig> {
	if (typeof value !== "object" || value === null || Array.isArray(value)) {
		return { ok: false, error: "expected object with contextWindowThreshold, above, below" };
	}
	const raw = value as Record<string, unknown>;
	const knownKeys = new Set(["contextWindowThreshold", "above", "below"]);
	for (const key of Object.keys(raw)) {
		if (!knownKeys.has(key)) {
			return { ok: false, error: `unknown key: ${key}` };
		}
	}
	if (!("contextWindowThreshold" in raw)) {
		return { ok: false, error: 'missing required "contextWindowThreshold"' };
	}
	if (!("above" in raw)) {
		return { ok: false, error: 'missing required "above"' };
	}
	if (!("below" in raw)) {
		return { ok: false, error: 'missing required "below"' };
	}
	const threshold = parseNonNegativeInt(raw.contextWindowThreshold);
	if (!threshold.ok) return { ok: false, error: `contextWindowThreshold: ${threshold.error}` };
	const above = parseNonNegativeInt(raw.above);
	if (!above.ok) return { ok: false, error: `above: ${above.error}` };
	const below = parseNonNegativeInt(raw.below);
	if (!below.ok) return { ok: false, error: `below: ${below.error}` };
	return { ok: true, value: { contextWindowThreshold: threshold.value, above: above.value, below: below.value } };
}
