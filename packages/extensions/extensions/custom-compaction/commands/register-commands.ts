...
function formatTrigger(policy: {
	trigger: { maxTokens?: number; autoMaxTokens?: { contextWindowThreshold: number; above: number; below: number }; minTokens: number; cooldownMs: number };
}): string {
	const { maxTokens, autoMaxTokens, minTokens, cooldownMs } = policy.trigger;
	let threshold: string;
	if (autoMaxTokens) {
		threshold = `adaptive (> ${autoMaxTokens.contextWindowThreshold}: ${autoMaxTokens.above}, else ${autoMaxTokens.below})`;
	} else {
		threshold = maxTokens && maxTokens > 0 ? `${maxTokens} tokens` : "inherited from pi";
	}
