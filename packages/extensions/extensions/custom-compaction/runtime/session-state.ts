...
export function createRuntimeServices(): RuntimeServices {
	...
	function updateStatus(ctx: ExtensionContext, policy: CompactionPolicy) {
		...
		const effectiveMax = computeEffectiveMaxTokens(policy.trigger, usage.contextWindow);
		const limit = effectiveMax && effectiveMax > 0 && effectiveMax < usage.contextWindow
			? effectiveMax
			: usage.contextWindow;
