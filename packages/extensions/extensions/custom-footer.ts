...
export default function (pi: ExtensionAPI) {
	...
	/** Timing for current agent run (used to compute tokens/sec). */
	let agentStartMs: number | null = null;
	/** Latest computed throughput snapshot from the most recent agent run. */
	let latestTps: TpsSnapshot | null = null;
				...
				render(width: number): string[] {
					...
					const contextUsage =
						usage?.tokens == null || usage?.contextWindow == null
							? `${pct.toFixed(0)}%`
							: `${pct.toFixed(0)}% ${fmt(usage.tokens)}/${fmt(usage.contextWindow)}`;

					const tokenStats = [
						theme.fg("accent", `${fmt(usageTotals.input)}/${fmt(usageTotals.output)}`),
						theme.fg("warning", `$${usageTotals.cost.toFixed(2)}`),
						theme.fg(pctColor, contextUsage),
					].join(" ");

					// Compaction status from custom-compaction extension
					const extStatuses = footerData.getExtensionStatuses();
					const compactStatus = extStatuses.get("compact-policy");
					const compactStr = compactStatus ? theme.fg("dim", compactStatus) : "";
					...
					const tpsStr = latestTps ? theme.fg("success", `⚡ ${latestTps.tokensPerSecond.toFixed(1)} t/s`) : "";

					const sep = theme.fg("dim", "  |  ");
					const leftParts = [modelStr, tokenStats];
					if (compactStr) {
						leftParts.push(compactStr);
					}
					if (tpsStr) {
						leftParts.push(tpsStr);
					}
					leftParts.push(elapsed, cwdStr);
					if (branchStr) {
	...
	function buildStatusLines(theme: { fg: (color: any, text: string) => string }): string[] {
		const lines: string[] = [];
		const sep = theme.fg("dim", " │ ");
		...
		if (latestTps) {
			lines.push(
				`  ${theme.fg("accent", "Throughput")}${sep}${theme.fg("success", `${latestTps.tokensPerSecond.toFixed(1)} tok/s`)}${sep}${theme.fg("dim", `${latestTps.output.toLocaleString()} out / ${latestTps.elapsedSeconds.toFixed(1)}s`)}`,
			);
		}
