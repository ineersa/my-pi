...
const INTERCOM_SEND_RESULT_EVENT = "pi-intercom:send-result";
...
const SILENT_SUBAGENT_PROTOCOL_PREFIX = "PI_SUBAGENTS_EVENT:";

interface IntercomEventSendRequest {
  requestId?: string;
  to?: string;
  message?: string;
...
function isSilentProtocolMessage(message: Message): boolean {
  return message.content.text.startsWith(SILENT_SUBAGENT_PROTOCOL_PREFIX);
}
...
export default function piIntercomExtension(pi: ExtensionAPI) {
  ...
  function emitReady(activeClient: IntercomClient): void {
    if (!runtimeContext || !currentSessionId || !activeClient.sessionId) {
      return;
    }
    pi.events.emit(INTERCOM_READY_EVENT, {
  ...
  function handleIncomingMessage(ctx: ExtensionContext, from: SessionInfo, message: Message): void {
    ...
    pi.events.emit(INTERCOM_INCOMING_EVENT, { from, message });

    if (isSilentProtocolMessage(message)) {
      return;
    }
  ...
  async function handleEventSendRequest(payload: unknown): Promise<void> {
    if (!payload || typeof payload !== "object") {
      return;
    }
    const req = payload as IntercomEventSendRequest;
    ...
    if (!to || !message) {
      pi.events.emit(INTERCOM_SEND_RESULT_EVENT, {
        requestId,
        delivered: false,
        reason: "Missing destination or message",
      });
      return;
    }
    ...
    try {
      ...
      const sessionIdForPresence = runtimeContext?.sessionManager.getSessionId() ?? currentSessionId;
      if (sessionIdForPresence) {
        syncPresenceIdentity(sessionIdForPresence);
      }
  ...
  pi.events.on(INTERCOM_SEND_REQUEST_EVENT, (payload) => {
    void handleEventSendRequest(payload);
  });
...
Usage:
  ...
  pi.registerCommand("intercom", {
    ...
    handler: async (_args, ctx) => openIntercomOverlay(ctx as ExtensionContext),
  });
