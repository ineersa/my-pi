...
export function registerEvents(pi: ExtensionAPI, runtime: RuntimeServices): void {
	...
	pi.on("session_before_compact", async (event, ctx) => {
		...
		if (!compactionPreparation.preparation) {
			return fallbackToDefaultCompaction("Compaction preparation is unexpectedly undefined. Falling back to Pi default compaction.");
		}

		const preparation = compactionPreparation.preparation;
